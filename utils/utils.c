...
void *map_anon_exec(__u64 sz) {
	void *map = mmap(0, sz, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
	memset(map, 0, sz);
	return map;
}
...
void *map_replace(void *map, int prot, __u64 size, __u64 add) {
	void *new = mmap(0, size+add, prot, MAP_ANON|MAP_PRIVATE, -1, 0);
	if (!new) return NULL;
